...
Tale cifrario è stato integrato a basso livello nella libreria C \textsc{MAVLink} allo scopo di cifrare il payload (già serializzato e quindi configurato come \textbf{sequenza di bytes}) dei messaggi scambiati tra le due parti che partecipano alla comunicazione.

È stato quindi proposto un vero e proprio \textbf{protocollo} per la definizione e la condivisione della chiave.

Per definire quest'architettura è stato necessario procedere per step:

\begin{enumerate}
    \item Definizione di nuovi messaggi \textsc{MAVLink} 2.0;
    \item Compilazione ed installazione di un'implementazione di\texttt{Kyber} nell'ambiente di sviluppo;
    \item Integrazione di \texttt{Kyber} all'interno di ArduPilot;
    \item Alterazione di \textbf{ArduPilot};
    \item Alterazione di \textbf{QGroundControl};
