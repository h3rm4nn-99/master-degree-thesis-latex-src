...
Oltre alla definizione di funzioni che modellano l'invio di messaggi \textsc{MAVLink}, è stato necessario codificare delle funzioni che modellassero la \textbf{ricezione} dei \textbf{messaggi di acknowledgement}, ovvero \texttt{KEYEXCHANGEGCSACK} e \texttt{CAPSULEACK}.
...
In quest'ultimo caso, quando la capsual è stata scambiata, viene impostata anche la \textbf{chiave di sessione}, generata in precedenza mediante chiamata ad \texttt{Encaps()}: da questo momento in poi è possibile iniziare la \textbf{comunicazione cifrata} tra l'UAV e la GCS.
